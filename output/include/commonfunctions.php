...
function checkTableName($shortTName, $type=false)
{
	...
	if ("exam_marking" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	...
	if ("setting" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("setting_category" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	...
	if ("admin_users" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("analisa_kelulusan_psra_sek" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("gps_psra_sek" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("analisa_subjek_psra_sek" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("purata_psra_sek" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("analisa_kelulusan_psra_daerah" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("gps_psra_daerah" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("analisa_subjek_psra_daerah" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("purata_psra_daerah" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("analisa_kelulusan_psra_all" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("gps_psra_all" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("analisa_subjek_psra_all" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	...
	if ("archive_exam_marking" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("result_psra" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("semakan_duplicate" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("daftar_sek" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("daftar_daerah" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("daftar_all" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("pendaftaran_murid" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("pendaftaran_sekolah" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("marking_AS" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("marking_BA" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("marking_JIK" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("marking_TF" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("marking_TJ" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
...
{
	$arr = array();
	$strPerm = GetUserPermissions("exam_marking");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="exam_marking";
	}
	$strPerm = GetUserPermissions("semakan");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="semakan";
	}
	...
	$strPerm = GetUserPermissions("setting_category");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="setting_category";
	}
	...
	$strPerm = GetUserPermissions("analisa_kelulusan_psra_sek");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="analisa_kelulusan_psra_sek";
	}
	$strPerm = GetUserPermissions("gps_psra_sek");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="gps_psra_sek";
	}
	$strPerm = GetUserPermissions("analisa_subjek_psra_sek");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="analisa_subjek_psra_sek";
	}
	$strPerm = GetUserPermissions("purata_psra_sek");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="purata_psra_sek";
	}
	$strPerm = GetUserPermissions("analisa_kelulusan_psra_daerah");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="analisa_kelulusan_psra_daerah";
	}
	$strPerm = GetUserPermissions("gps_psra_daerah");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="gps_psra_daerah";
	}
	$strPerm = GetUserPermissions("analisa_subjek_psra_daerah");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="analisa_subjek_psra_daerah";
	}
	$strPerm = GetUserPermissions("purata_psra_daerah");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="purata_psra_daerah";
	}
	$strPerm = GetUserPermissions("analisa_kelulusan_psra_all");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="analisa_kelulusan_psra_all";
	}
	$strPerm = GetUserPermissions("gps_psra_all");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="gps_psra_all";
	}
	$strPerm = GetUserPermissions("analisa_subjek_psra_all");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="analisa_subjek_psra_all";
	}
	$strPerm = GetUserPermissions("purata_psra_all");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="purata_psra_all";
	}
	...
	$strPerm = GetUserPermissions("archive_exam_marking");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="archive_exam_marking";
	}
	$strPerm = GetUserPermissions("result_psra");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="result_psra";
	}
	$strPerm = GetUserPermissions("semakan_duplicate");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="semakan_duplicate";
	}
	$strPerm = GetUserPermissions("daftar_sek");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="daftar_sek";
	}
	$strPerm = GetUserPermissions("daftar_daerah");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="daftar_daerah";
	}
	$strPerm = GetUserPermissions("daftar_all");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="daftar_all";
	}
	$strPerm = GetUserPermissions("pendaftaran_murid");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="pendaftaran_murid";
	}
	$strPerm = GetUserPermissions("pendaftaran_sekolah");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="pendaftaran_sekolah";
	}
	$strPerm = GetUserPermissions("marking_AS");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="marking_AS";
	}
	$strPerm = GetUserPermissions("marking_BA");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="marking_BA";
	}
	$strPerm = GetUserPermissions("marking_JIK");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="marking_JIK";
	}
	$strPerm = GetUserPermissions("marking_TF");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="marking_TF";
	}
	$strPerm = GetUserPermissions("marking_TJ");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="marking_TJ";
	}
	$strPerm = GetUserPermissions("marking_UTH");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="marking_UTH";
	}
...
{	
	...
	if(!count($_SESSION["UserRights"]["Guest"]))
		return false;
	if(array_key_exists("exam_marking",$_SESSION["UserRights"]["Guest"]))
		return true;
	...
	if(array_key_exists("setting",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("setting_category",$_SESSION["UserRights"]["Guest"]))
		return true;
	...
	if(array_key_exists("admin_users",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("analisa_kelulusan_psra_sek",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("gps_psra_sek",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("analisa_subjek_psra_sek",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("purata_psra_sek",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("analisa_kelulusan_psra_daerah",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("gps_psra_daerah",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("analisa_subjek_psra_daerah",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("purata_psra_daerah",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("analisa_kelulusan_psra_all",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("gps_psra_all",$_SESSION["UserRights"]["Guest"]))
		return true;
	...
	if(array_key_exists("dashboard_utama",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("archive_exam_marking",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("result_psra",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("semakan_duplicate",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("daftar_sek",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("daftar_daerah",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("daftar_all",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("pendaftaran_murid",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("pendaftaran_sekolah",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("marking_AS",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("marking_BA",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("marking_JIK",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("marking_TF",$_SESSION["UserRights"]["Guest"]))
		return true;
...
function SetAuthSessionData($pUsername, &$data, $fromFacebook, $password, &$pageObject = null)
{
	...
	$_SESSION["GroupID"] = $data["username"];
		...
		$_SESSION["OwnerID"] = $data["fullname"];
	$_SESSION["_result_psra_OwnerID"] = $data["fullname"];
